...
namespace MvcSf.Controllers
{
    public class ShoppingCartProductTableController : Controller
    {
        ...
        public ActionResult testingJoinWithPeriod()
        {
            var userQuery = db.UserTable.Where(u => u.UserName == System.Web.HttpContext.Current.User.Identity.Name).Select(u => u).FirstOrDefault();
            var cartQuery = userQuery.ShoppingCartTable.FirstOrDefault();
            var itemQuery = cartQuery.ShoppingCartProductTable.FirstOrDefault();
            var itemQuery2 = db.UserTable.Where(u => u.UserName == System.Web.HttpContext.Current.User.Identity.Name).Select(u => u).FirstOrDefault().ShoppingCartTable.FirstOrDefault().ShoppingCartProductTable.FirstOrDefault();
            return Content("This is how you chain multiple LINQ queries!");
        }
        ...
        public ActionResult AddToCart(int id)
        {
            ...
            if (!(matchCartItemToCart == null))
            {
                ...
                matchCartItemToCart.Price = matchCartItemToCart.Quantity * productTemp.Price;
        ...
        public void EmptyTheCart()
        {
            ...
            {
                db.ShoppingCartProductTable.Remove(matchCartItemToCart.ToList()[i]);
            }
            db.SaveChanges();
        }
        ...
        */
        ...
        public ActionResult UpdateQuantity(int id, int quantity)
        {
            ...
            cartItem.Quantity = quantity;
            var itemType = (from qualityQualifier in db.ProductTable.Where(QQ => QQ.ProductID == id) select qualityQualifier).FirstOrDefault();
            if(quantity > itemType.Quantity)
            {
                quantity = (int)itemType.Quantity;
            }
            else
            {
                if (quantity < 0)
            ...
            if (quantity < 1)
            {
                db.ShoppingCartProductTable.Remove(cartItem);
                db.SaveChanges();
                return Json(new List<string> { "0", ((int)((totalQuantity != null) ? totalQuantity : 0)).ToString(), ((decimal)cartItem.Price).ToString("C2"), ((decimal)getPrice()).ToString("C2") });
            }
            else
            {
                cartItem.Price = itemType.Price * cartItem.Quantity;
                db.Entry(cartItem).State = EntityState.Modified;
                db.SaveChanges();
                ViewBag.Price = cartItem.Price;
                ViewBag.TotalPrice = getPrice();
                return Json(new List<string> { ((int)quantity).ToString(), ((int)totalQuantity).ToString(), ((decimal)cartItem.Price).ToString("C2"), ((decimal)getPrice()).ToString("C2") });
            }
            //}
            //catch
            //{
